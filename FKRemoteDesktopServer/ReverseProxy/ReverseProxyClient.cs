...
namespace FKRemoteDesktop.ReverseProxy
{
    public class ReverseProxyClient
    {
        ...
        public int ConnectionId { get { return Handle.Handle.ToInt32(); } }
        ...
        public EProxyType Type { get; private set; }
        ...
        public ReverseProxyClient(Client client, Socket socket, ReverseProxyServer server)
        {
            ...
            this.TargetServer = "";
            this.Type = EProxyType.eProxyType_Unknown;
            this.Server = server;
            try
            {
        ...
        private void AsyncReceive(IAsyncResult ar)
        {
            try
            {
                ...
                {
                    // 防止 HandshakeStream 溢出攻击，正常情况下，Socks或HTTPS代理用不了5000个初始数据包
                    Disconnect();
                    return;
                }
                ...
                case 0:
                    {   // 初始化Sock包
                        if (payload.Length >= 3)
                        {
                            ...
                            {
                                Type = EProxyType.eProxyType_SOCKS5;
                            }
                            else if (headerStr.StartsWith("CONNECT") && headerStr.Contains(":"))
                            {
                                Type = EProxyType.eProxyType_HTTPS;
        ...
        {
            if (Type == EProxyType.eProxyType_HTTPS)
                Disconnect();
            if (Type == EProxyType.eProxyType_SOCKS5)
            {
        ...
        public void HandleCommandResponse(ReverseProxyConnectResponse response)
        {
            if (!_receivedConnResponse)
            {
                ...
                if (response.IsConnected)
                {
                    ...
                    else if (Type == EProxyType.eProxyType_SOCKS5)
                    {
                        try
                        {
