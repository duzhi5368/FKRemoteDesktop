...
namespace FKRemoteDesktop.Utilities
{
    public abstract class JsonValue : IEnumerable
    {
        ...
        public abstract EJsonType JsonType
        {
            get;
        }
        ...
        public static JsonValue Load(TextReader textReader)
        {
            ...
            return ToJsonValue(new JsonReader(textReader).Read());
        }
        ...
        public static JsonValue ToJsonValue<T>(T ret)
        {
            ...
            throw new NotSupportedException($"解析器得到意外的类型: {ret.GetType()}");
        }
        ...
        private void SavePublic(TextWriter w, bool saving)
        {
            switch (JsonType)
            {
                case EJsonType.eJsonType_Object:
                    {
                        w.Write('{');
                        bool flag = false;
                ...
                case EJsonType.eJsonType_Array:
                    {
                        w.Write('[');
                        bool flag = false;
        ...
        private bool NeedEscape(string src, int i)
        {
            char c = src[i];
            if (c >= ' ' && c != '"' && c != '\\' && (c < '\ud800' || c > '\udbff' ||
                (i != src.Length - 1 && src[i + 1] >= '\udc00' && src[i + 1] <= '\udfff')) && (c < '\udc00' ||
                c > '\udfff' || (i != 0 && src[i - 1] >= '\ud800' && src[i - 1] <= '\udbff')) &&
                c != '\u2028' && c != '\u2029')
            {
                if (c == '/' && i > 0)
                {
